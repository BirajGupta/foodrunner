...
class Allrestadapter(val context: Context,val data: List<Restaurant>):RecyclerView.Adapter<Allrestadapter.AllrestadapterViewHolder>() {
    ...
    override fun onBindViewHolder(holder: AllrestadapterViewHolder, position: Int) {
       var info:Restaurant=data[position]
        ...
        val restaurantEntity=RestaurantEntity(info.id,info.name,info.rating,info.cost,info.img)
    ...
    class DBAsynctask(val context: Context,val restaurantEntity: RestaurantEntity,val mode:Int):AsyncTask<Void,Void,Boolean>(){
        val db= Room.databaseBuilder(context,Database::class.java,"db").addMigrations(MIGRATION_1_2).build()
        override fun doInBackground(vararg params: Void?): Boolean
        {
            when(mode){
                1->{
                  val restaurantEntity1:RestaurantEntity  ?=db.restdao().getbyid(restaurantEntity.id)
                    db.close()
